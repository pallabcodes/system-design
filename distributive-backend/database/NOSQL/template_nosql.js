 ...
 */
 ...
 //$avg is available in these stages: $addFields, $bucket, $bucketAuto, $group, $match, $project, $replaceRoot, $set, $setWindowFields
 
 db.getCollection('sales4').find();
    ...
    [
       {
          ...
          login: ISODate("2021-03-13T10:00:00Z"), // 'Z' indicates UTC time
          logout: ISODate("2021-03-14T18:00:00Z") // 'Z' indicates UTC time
       },
       {
          location: "America/Mexico_City",
          login: ISODate("2021-03-13T10:00:00Z"), // 'Z' indicates UTC time
          logout: ISODate("2021-03-14T08:00:00Z") // 'Z' indicates UTC time
       }
    ]
 );
 
 db.getCollection('billing').find();
       ...
       $project: {
          date: {
             $dateFromString: {
                dateString: '$date',
                timezone: '$timezone',
                onError: '$date' // this will keep the original value in case of error
             }
          }
       }
    }
       ...
       $project: {
          date: {
             $dateFromString: {
                dateString: '$date',
                timezone: '$timezone',
                onError: new Date(0) // Fallback to this date if an error occurs during parsing
             }
          }
       }
    }
 ...
 db.connectionTime.find({}, { _id: 0, custId: 1, logout: 1 }).pretty();
 ...
 db.connectionTime.aggregate([
    {
       $match: {
          $expr: {
             $lt: [
                "$logoutTime",
                new Date()  // Check if logoutTime is before the current date/time
             ]
          }
       }
    },
 ...
 db.players.aggregate([
    {
       $addFields: {
          ...
          isFound: {
             $eq: [
                { $function: { body: "hex_md5('$name')", args: [], lang: "js" } },
 ...
 // Suppose the collection looks like this:
 ...
 //[
 //   { "_id": 1, "name": "Miss Cheevous", "scores": [10, 5, 10], "testField": "MISS CHEEVOUS" },
 //   { "_id": 2, "name": "Miss Ann Thrope", "scores": [10, 10, 10], "testField": "MISS ANN THROPE" }
 //]
 
 db.players.find({
    $expr: {
       $function: {
          body: "function(name) { return hex_md5(name) == '15b0a220baa16331e8d80e15367677ad'; }", // Function as a string
          args: ["$name"], // Use the document field name
          lang: "js"
       }
    }
 });
 ...
 db.getCollection('inventory5').find();
    ...
    [
       {
          $project: {
             item: 1,
             // so , if field i.e. 'value' null/non-existent then use `description (if available)`, else `$quantity`
