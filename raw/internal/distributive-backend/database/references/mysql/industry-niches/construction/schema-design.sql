-- Construction & Project Management Platform Database Schema
...
-- Configure database for construction compliance
ALTER DATABASE ConstructionDB
SET
    RECOVERY SIMPLE,
...
SELECT
    ...
    p.BudgetConsumed,
    CASE
        WHEN p.TotalBudget > 0 THEN (p.BudgetConsumed / p.TotalBudget) * 100
        ELSE 0
    ...
    CASE
        ...
        WHEN p.ActualCompletionDate > p.EndDate THEN 'Delayed'
        WHEN GETDATE() > p.EndDate THEN 'Overdue'
        WHEN p.ProgressPercentage < 25 AND GETDATE() > DATEADD(DAY, 30, p.StartDate) THEN 'At Risk'
        ELSE 'On Track'
    END AS TimelineStatus,

    -- Contractor count
    (SELECT COUNT(*) FROM ProjectContractors pc WHERE pc.ProjectID = p.ProjectID AND pc.Status = 'Active') AS ActiveContractors,

    -- Safety incidents
    (SELECT COUNT(*) FROM SafetyIncidents si WHERE si.ProjectID = p.ProjectID AND si.IncidentDate >= p.StartDate) AS SafetyIncidents
...
CREATE VIEW vw_ContractorPerformance
AS
SELECT
    c.ContractorID,
    c.ContractorNumber,
    ...
    c.CompletedProjects,
    CASE
        WHEN c.TotalProjects > 0 THEN (c.CompletedProjects * 1.0 / c.TotalProjects) * 100
        ELSE 0
    END AS CompletionRate,

    -- Current active projects
    (SELECT COUNT(*) FROM ProjectContractors pc WHERE pc.ContractorID = c.ContractorID AND pc.Status = 'Active') AS ActiveProjects,
    ...
    (SELECT AVG(pc.PerformanceRating) FROM ProjectContractors pc
     WHERE pc.ContractorID = c.ContractorID AND pc.Status = 'Completed'
     AND pc.EndDate >= DATEADD(MONTH, -6, GETDATE())) AS AvgRecentRating
...
CREATE TRIGGER TR_WBSItems_UpdateProjectProgress
ON WBSItems
...
BEGIN
    ...
    -- Update project progress percentage based on completed WBS items
    UPDATE p
    SET p.ProgressPercentage = (
        SELECT AVG(wi.ProgressPercentage)
        FROM WBSItems wi
        WHERE wi.ProjectID = p.ProjectID
        AND wi.ParentWBSID IS NULL -- Only top-level items
    )
    FROM Projects p
...
-- Update contractor project counts
CREATE TRIGGER TR_ProjectContractors_UpdateCounts
ON ProjectContractors
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ...
    UPDATE c
    SET c.TotalProjects = (
            SELECT COUNT(*) FROM ProjectContractors pc WHERE pc.ContractorID = c.ContractorID
        ),
        c.CompletedProjects = (
            SELECT COUNT(*) FROM ProjectContractors pc
            WHERE pc.ContractorID = c.ContractorID AND pc.Status = 'Completed'
        )
    FROM Contractors c
    WHERE c.ContractorID IN (
        SELECT DISTINCT ContractorID FROM inserted
        UNION
        SELECT DISTINCT ContractorID FROM deleted
    );
END;
GO
...
BEGIN
    ...
    SET @ProjectNumber = 'PROJ-' + CAST(YEAR(GETDATE()) AS NVARCHAR(4)) +
                        RIGHT('00' + CAST(MONTH(GETDATE()) AS NVARCHAR(2)), 2) + '-' +
                        RIGHT('0000' + CAST(ABS(CHECKSUM(NEWID())) % 10000 AS NVARCHAR(4)), 4);

    INSERT INTO Projects (
        ProjectNumber, ProjectName, ProjectType, Description,
        StartDate, EndDate, TotalBudget, ClientID, ProjectManagerID
    )
    VALUES (
        @ProjectNumber, @ProjectName, @ProjectType, @Description,
        @StartDate, @EndDate, @TotalBudget, @ClientID, @ProjectManagerID
    ...
    SELECT SCOPE_IDENTITY() AS ProjectID, @ProjectNumber AS ProjectNumber;
END;
GO

-- Report safety incident procedure
CREATE PROCEDURE sp_ReportSafetyIncident
    @ProjectID INT,
    @IncidentType NVARCHAR(50),
    @Description NVARCHAR(MAX),
    @ReportedBy INT,
    @Location NVARCHAR(200) = NULL,
    @Severity NVARCHAR(20) = 'Minor'
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SET @IncidentNumber = 'SAFE-' + CAST(YEAR(GETDATE()) AS NVARCHAR(4)) +
                         RIGHT('00' + CAST(MONTH(GETDATE()) AS NVARCHAR(2)), 2) + '-' +
                         RIGHT('00000' + CAST(ABS(CHECKSUM(NEWID())) % 100000 AS NVARCHAR(5)), 5);

    INSERT INTO SafetyIncidents (
        IncidentNumber, ProjectID, IncidentType, Description,
        ReportedBy, Location, Severity, IncidentDate
    )
    VALUES (
        @IncidentNumber, @ProjectID, @IncidentType, @Description,
        @ReportedBy, @Location, @Severity, GETDATE()
    );
...
INSERT INTO Projects (ProjectNumber, ProjectName, ProjectType, StartDate, EndDate, TotalBudget) VALUES
('PROJ-202412-0001', 'Office Building Construction', 'Construction', '2024-01-01', '2024-12-31', 2500000.00);

-- Insert sample contractor
INSERT INTO Contractors (ContractorNumber, CompanyName, BusinessType, Rating) VALUES
('CON-000001', 'ABC Construction LLC', 'General', 4.5);

-- Insert sample equipment
INSERT INTO Equipment (EquipmentNumber, EquipmentName, EquipmentType, Manufacturer) VALUES
('EQ-001', 'Excavator CAT 320', 'HeavyEquipment', 'Caterpillar');

-- Insert sample material
INSERT INTO Materials (MaterialCode, MaterialName, MaterialType, UnitOfMeasure, UnitCost) VALUES
('MAT-001', 'Portland Cement', 'Construction', 'Bag', 12.50);

-- Insert sample safety incident
INSERT INTO SafetyIncidents (IncidentNumber, ProjectID, IncidentType, Description, ReportedBy, Severity) VALUES
('SAFE-202412-00001', 1, 'NearMiss', 'Worker nearly struck by falling debris', 1, 'Minor');

PRINT 'Construction database schema created successfully!';
GO
